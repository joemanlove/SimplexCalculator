...
class SimplexSolve:
    ...
    def solve(self) -> None:
        ...
        while is_negative(matrix[-1]):
            ...
            for row_vector in matrix[:pivot_row]+matrix[pivot_row+1:]:
                ...
                for i, component in enumerate(row_vector):
                    component += (-1)*matrix[pivot_row][i]*val
            # Save completed step.
    ...
    def find_pivot(self, matrix: list) -> tuple:
        ...
        divided_col = [x[-1] / x[pivot_col] for x in matrix[:-1]]
        # Get the index of the smallest value
        pivot_row = divided_col.index(min(divided_col))
